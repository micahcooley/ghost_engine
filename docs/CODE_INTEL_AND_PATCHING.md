...
Phase 11A adds explicit reviewed negative-knowledge persistence through GIP `negative_knowledge.review`, plus read-only inspection through `negative_knowledge.reviewed.list` and `negative_knowledge.reviewed.get`. Negative-knowledge candidates remain signal until explicitly reviewed. Accepted and rejected reviewed NK records are appended under the same project shard at `negative_knowledge/reviewed_negative_knowledge.jsonl`, with no rewrite, deletion, compaction, hidden promotion, corpus mutation, pack mutation, command execution, or verifier execution. Accepted reviewed NK remains `nonAuthorizing:true`, `treatedAsProof:false`, `usedAsEvidence:false`, and `globalPromotion:false`.

Phase 11B lets accepted reviewed negative knowledge influence `corpus.ask` and `rule.evaluate` in the same project shard as non-authorizing warnings, exact repeated-pattern suppression, lower-ranking/penalty metadata, stronger-exact-evidence requirements, verifier/check candidates, or future pack/corpus/rule update candidates. The reader is bounded and read-only, missing storage is no influence, malformed JSONL lines become `acceptedNegativeKnowledgeWarnings` / `negativeKnowledgeTelemetry`, and rejected reviewed NK records do not influence. Matching is conservative and structural only: operation kind when present, exact ids, exact/substring text, source/evidence/rule/output fields, and deterministic fingerprints. It does not use semantic search, embeddings, Transformers, model adapters, cloud calls, or network calls. Reviewed NK never enters `evidenceUsed`, never proves or supports an answer, never discharges rule proof/support gates, never executes verifiers or commands, never mutates corpus, packs, corrections, or NK records, and never promotes globally or across unrelated shards.

Phase 12A adds read-only learning-loop status through GIP `learning.status`. The operation is a same-shard scoreboard over `corrections/reviewed_corrections.jsonl` and `negative_knowledge/reviewed_negative_knowledge.jsonl`: reviewed/accepted/rejected counts, operation and influence-kind counts, future behavior/suppression/stronger-evidence/verifier/pack/corpus/rule candidate counts, malformed-line warnings, capacity telemetry, storage flags, and optional bounded sampled records. It tolerates missing files, never rewrites/compacts/deletes JSONL, never auto-reviews or auto-accepts anything, never executes commands or verifiers, and never mutates corpus, packs, correction records, or negative knowledge. The scoreboard is diagnostics only: it is not proof, evidence, support, global promotion, hidden learning, or semantic matching.

Phase 13A adds procedure pack candidate lifecycle operations through GIP `procedure_pack.candidate.propose`, `procedure_pack.candidate.review`, `procedure_pack.candidate.reviewed.list`, and `procedure_pack.candidate.reviewed.get`. Proposal turns same-shard reviewed corrections, reviewed negative knowledge, or `learning.status` summaries into explicit structured procedure candidates with non-executable steps, required evidence, triggers, safety boundaries, `nonAuthorizing:true`, `treatedAsProof:false`, `executesByDefault:false`, `packMutation:false`, and `globalPromotion:false`. Proposals are non-persistent unless explicitly reviewed. Review appends accepted/rejected candidate records under `procedure_packs/reviewed_pack_candidates.jsonl`; list/get inspect that file read-only. None of these operations mutate packs, auto-promote candidates, globally promote anything, execute commands/verifiers, or use candidates as proof/evidence.
