...
- `corpus.ask` carries per-item `licenseStatus` and `authorityLevel` from the source corpus `license.json`. If verified evidence contradicts unverified evidence, the unverified evidence is listed under `suppressedEvidence` and excluded from answer drafting. Drafts based on unverified sources are prefixed with `⚠️ Unverified Source`. Rank-3 Shadow evidence can be returned only as context; if only Shadow evidence matches, `corpus.ask` stays unresolved with `insufficient_high_rank_evidence` and no `answerDraft`. Rank-4 Trash evidence is blacklisted: it is never returned in `evidenceUsed`, appears only as blocked `suppressedEvidence`, and Trash-only exact matches return `status:"rejected_falsehood"`.
...
- `corpus.ask` — Answer from explicitly ingested live shard corpus evidence **(Implemented)**
  ...
  - **Request**: `{"question": string}` or `{"message": string}`, plus optional `projectShard` / `project_shard`, `mountedPacks` / `mounted_packs`, `maxResults` / `max_results`, `maxSnippetBytes` / `max_snippet_bytes`, and `requireCitations` / `require_citations`. `mountedPacks` is an explicit read-only composition list of installed Knowledge Pack refs, either `{"packId":"<id>","packVersion":"<version>"}` objects or `"pack@version"` strings; omitted `packVersion` defaults to `v1`.
  ...
  - Uses only existing live corpus state created through explicit corpus ingestion/lifecycle paths plus any request-explicit installed Knowledge Packs listed in `mountedPacks`. Staged corpus is not read as active knowledge, and mounted pack reads do not merge or copy pack files into the project shard.
  ...
  - `evidenceUsed` reports corpus lineage id/path/source path/source label/class/trust class/license status/authority level, content hash, byte and line spans, bounded snippet with truncation flag, matched terms/phrase, match reason, provenance, score, and rank.
  ...
  - Trace includes `mountedPacksConsidered` and `mountedPackEntriesConsidered` when composition is requested. Trace flags always report `corpusMutation:false`, `packMutation:false`, `negativeKnowledgeMutation:false`, `commandsExecuted:false`, and `verifiersExecuted:false`.
