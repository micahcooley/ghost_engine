...
Accepted reviewed corrections are not an autopsy proof source. They may influence `corpus.ask` and `rule.evaluate` in their own same-shard runtime surfaces as warnings, exact repeated-output suppression, or future behavior candidates, and `correction.influence.status` may summarize those reviewed-record diagnostics read-only, but those signals remain non-authorizing and do not mutate packs, negative knowledge, corpus, correction records, or autopsy state. Reviewed negative knowledge has its own explicit `negative_knowledge.review` lifecycle and append-only `negative_knowledge/reviewed_negative_knowledge.jsonl` storage. Accepted reviewed NK may now influence `corpus.ask` and `rule.evaluate` in those operations' same-shard runtime surfaces as non-authorizing warnings, suppression, stronger-evidence/verifier requirements, or future behavior candidates, but it is still not proof or evidence and Context Autopsy does not apply it. `learning.status` can summarize same-shard reviewed correction and reviewed NK scoreboard counts read-only, but those counts are diagnostics only and never become autopsy proof, evidence, mutation, hidden learning, or global promotion. Procedure pack candidate operations may propose/review/inspect non-executable candidate procedures from reviewed learning signals, but those candidates are not autopsy proof or evidence, do not mutate packs, and do not execute commands or verifiers.
...
- **No hidden learning:** User corrections against Context Autopsy output are routed through `correction.propose`; they remain non-authorizing, `treatedAsProof:false`, and review-required until `correction.review` explicitly accepts or rejects them into append-only reviewed records. Accepted reviewed corrections are still not proof. Phase 8 influence is currently integrated first into `corpus.ask`, where same-shard accepted records can warn, require stronger evidence, suppress exact repeated bad answer patterns, or emit candidate-only follow-ups without mutating corpus, packs, or negative knowledge. Context Autopsy itself remains read-only and does not execute or apply those candidates automatically.
- **Reviewed negative knowledge:** Negative-knowledge candidates require explicit `negative_knowledge.review` before they become durable reviewed NK records. The reviewed records are append-only, shard-local, non-authorizing, not proof, not evidence, and not global promotion. Accepted reviewed NK can influence later `corpus.ask` and `rule.evaluate` only through bounded same-shard structural matching as warnings, exact suppression, stronger-evidence/verifier requirements, or future behavior candidates. It never mutates corpus, packs, corrections, or NK records, never executes commands or verifiers, and never uses semantic matching. `negative_knowledge.reviewed.list` and `negative_knowledge.reviewed.get` remain inspection-only; Context Autopsy does not scan, promote, or apply reviewed NK automatically.
