...
pub const ReferenceInferenceHarness = struct {
    ...
    pub fn forward(self: ReferenceInferenceHarness, input_text: []const u8, session_id: u64) !ForwardSummary {
        ...
        // Initialize hidden state from initial context embedding resonance
        if (context.len > 0) {
        ...
        if (self.loader) |ld| {
            for (0..35) |layer_idx| {
                ...
                // Attends directly to the active resonance memory lattice (Meaning Matrix)
                // using true Q, K, V weight projections per layer.
                ...
                if (ld.getTensor(q_name)) |q_view| {
                    ...
                    // We project each memory context entry through K and V for all 8 heads
                    for (context, 0..) |entry, c_idx| {
                        ...
                        for (0..8) |head_idx| {
                            ...
                            // Numerically stable Softmax contribution
                            const s = @exp(score - max_score);
                            entry_total_s += s;
                            
                            for (0..256) |i| {
                                weighted_v[head_base + i] += v_full[head_base + i] * s;
                            }
                        }
                        scores[c_idx] = entry_total_s;
                    }

                    // Final normalization of weighted_v sum
                    var sum_exp: f32 = 0.0;
                ...
                const up_view = ld.getTensor(up_name) catch return error.MissingGGUFTensor;
                const gate_view = ld.getTensor(gate_name) catch return error.MissingGGUFTensor;
                const down_view = ld.getTensor(down_name) catch return error.MissingGGUFTensor;
        ...
        const out_rune = head.project(hidden);

        const phead = prose_head.ProseHead.init(self.allocator);
        const synthesized_prose = try phead.synthesize(out_rune, context);

        const state_msg = if (self.loader != null) "active_gguf_weights_authorizing_phase_harness" else HarnessState;
        return .{
            .state = state_msg,
