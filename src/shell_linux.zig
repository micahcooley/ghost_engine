...
const sigil_snapshot = core.sigil_snapshot;
...
const SEND_FLAGS: u32 = if (builtin.os.tag == .windows) 0 else posix.MSG.NOSIGNAL;
const SOCKET_NONBLOCK_FLAG: usize = 1 << @bitOffsetOf(posix.O, "NONBLOCK");
...
fn sendBytes(sock: usize, bytes: []const u8) void {
    ...
    _ = posix.send(@intCast(sock), bytes, SEND_FLAGS) catch {};
}

fn sendNonBlocking(sock: usize, bytes: []const u8, sent: *usize) !bool {
    if (sent.* >= bytes.len) return true;

    const n = posix.send(@intCast(sock), bytes[sent.*..], SEND_FLAGS) catch |err| switch (err) {
...
fn buildSigilStateJson(
    allocator: std.mem.Allocator,
    snapshot: sigil_runtime.ControlSnapshot,
    snap_status: sigil_snapshot.SnapshotStatus,
    command_name: ?[]const u8,
    extra_field_name: ?[]const u8,
    extra_json: ?[]const u8,
) ![]u8 {
    ...
    if (command_name) |name| {
        try appendPrint(&list, allocator, "{{\"status\":\"ok\",\"command\":\"{s}\",\"control\":{{\"reasoningMode\":\"{s}\",\"mood\":\"{s}\",\"saturationBonus\":{d},\"boredomPenaltyHigh\":{d},\"boredomPenaltyLow\":{d},\"enableVulkan\":{s},\"forceCpuOnly\":{s},\"lastScan\":{{\"hash\":{d},\"energy\":{d}}},\"lockedSlots\":{d},\"lockedHashes\":{d},\"bindings\":{d}}},\"snapshots\":{{\"scratchActive\":{s},\"committed\":{s},\"snapshot\":{s}}}}}", .{
            name,
            ...
            if (snap_status.scratch_active) "true" else "false",
            if (snap_status.committed_exists) "true" else "false",
            if (snap_status.snapshot_exists) "true" else "false",
        });
    } else try appendPrint(&list, allocator, "{{\"status\":\"ok\",\"control\":{{\"reasoningMode\":\"{s}\",\"mood\":\"{s}\",\"saturationBonus\":{d},\"boredomPenaltyHigh\":{d},\"boredomPenaltyLow\":{d},\"enableVulkan\":{s},\"forceCpuOnly\":{s},\"lastScan\":{{\"hash\":{d},\"energy\":{d}}},\"lockedSlots\":{d},\"lockedHashes\":{d},\"bindings\":{d}}},\"snapshots\":{{\"scratchActive\":{s},\"committed\":{s},\"snapshot\":{s}}}}}", .{
        sigil_runtime.reasoningModeName(snapshot.reasoning_mode),
        controlMoodName(snapshot),
        snapshot.saturation_bonus,
        snapshot.boredom_penalty_high,
    ...
    if (extra_json) |extra| {
        ...
        try list.appendSlice(extra_field_name orelse "result");
        try list.appendSlice("\":");
        try list.appendSlice(extra);
        try list.append('}');
    }
    return list.toOwnedSlice();
}

fn applySigilControlEffects(live: *engine.SingularityEngine, control: *sigil_runtime.ControlPlane) !void {
    const snapshot = control.snapshot();
    try live.setCacheCap(snapshot.lattice_cache_cap_bytes);
}

fn handleSigilCommandError(sock: usize, err: anyerror) void {
    switch (err) {
        error.ScratchAlreadyActive => sendJsonError(sock, .conflict, "Scratch session already active"),
        error.ScratchNotActive => sendJsonError(sock, .conflict, "Scratch session is not active"),
        error.ScratchSessionActive => sendJsonError(sock, .conflict, "Scratch session is active; commit or discard first"),
        error.ScratchMissing => sendJsonError(sock, .not_found, "Scratch baseline not found"),
        error.SnapshotMissing => sendJsonError(sock, .not_found, "Snapshot not found"),
        error.CommitMissing => sendJsonError(sock, .not_found, "Committed state not found"),
        error.ScratchCommitCollision => sendJsonError(sock, .conflict, "Scratch commit could not place all slots in permanent state"),
        error.InvalidControlFile, error.UnsupportedControlVersion, error.UnexpectedFileSize, error.ShortRead => sendJsonError(sock, .internal_server_error, "Snapshot data is corrupt"),
        else => sendJsonError(sock, .internal_server_error, "Sigil snapshot command failed"),
    }
}

fn handleAbstractionCommandError(sock: usize, err: anyerror) void {
    switch (err) {
        error.InvalidAbstractionCommand, error.InvalidRegionSpec => sendJsonError(sock, .bad_request, "Invalid abstraction command"),
        error.RegionOutOfRange => sendJsonError(sock, .bad_request, "Selected region is out of range"),
        error.RegionTooLarge, error.TooManySources, error.TooManyExamples => sendJsonError(sock, .bad_request, "Abstraction input exceeds bounded limits"),
        error.CodeIntelResultMissing => sendJsonError(sock, .not_found, "code_intel last_result.json was not found for the selected shard"),
        error.InvalidCodeIntelResult, error.InvalidAbstractionCatalog => sendJsonError(sock, .internal_server_error, "Abstraction data is corrupt"),
        error.NotEnoughExamples, error.NoConsensus, error.AbstractionBelowThreshold => sendJsonError(sock, .conflict, "Abstraction set did not reach deterministic commit thresholds"),
        error.CrossShardReuseRequiresProjectShard => sendJsonError(sock, .conflict, "Cross-shard reuse is only available while mounted on a project shard"),
        error.AbstractionSourceNotFound => sendJsonError(sock, .not_found, "Requested abstraction source was not found"),
        error.CrossShardConflictRequiresPromoteOrReject => sendJsonError(sock, .conflict, "Conflicting local abstraction requires promote or reject, not adopt"),
        error.MergeConflictRefused => sendJsonError(sock, .conflict, "Merge refused because the destination concept is incompatible"),
        error.MergeProvenanceRefused => sendJsonError(sock, .conflict, "Merge refused because provenance or trust rules were not satisfied"),
        error.SelfMergeNotAllowed => sendJsonError(sock, .conflict, "Merge source must be a different shard"),
        error.UsePromoteForHigherTrustDestination => sendJsonError(sock, .conflict, "Use promote when merging into a higher-trust destination"),
        error.TrustPromotionRequiresHigherDestination => sendJsonError(sock, .conflict, "Promote requires a strictly higher-trust destination"),
        error.ExploratoryContentCannotBePromoted => sendJsonError(sock, .conflict, "Exploratory content cannot be promoted into trusted memory"),
        error.PrunableContentCannotBePromoted => sendJsonError(sock, .conflict, "Prunable content cannot be promoted into trusted memory"),
        else => sendJsonError(sock, .internal_server_error, "Abstraction distillation failed"),
    }
}

fn handlePatchCandidateCommandError(sock: usize, err: anyerror) void {
    switch (err) {
        error.InvalidPatchCandidateCommand, error.InvalidArguments => sendJsonError(sock, .bad_request, "Invalid /stage_patch_candidates command"),
        error.SourceFileTooLarge => sendJsonError(sock, .bad_request, "Patch candidate source exceeded bounded limits"),
        else => sendJsonError(sock, .internal_server_error, "Patch candidate generation failed"),
    }
}
...
fn handleSigilRequest(sock: usize, allocator: std.mem.Allocator, body: []const u8) void {
    ...
    const parsed_command = sigil_snapshot.parseCommand(script);
    const abstraction_command = abstractions.isCommand(script);
    const reuse_command = std.mem.startsWith(u8, std.mem.trim(u8, script, " \r\n\t"), abstractions.REUSE_COMMAND_NAME);
    const merge_command = std.mem.startsWith(u8, std.mem.trim(u8, script, " \r\n\t"), abstractions.MERGE_COMMAND_NAME);
    const prune_command = std.mem.startsWith(u8, std.mem.trim(u8, script, " \r\n\t"), abstractions.PRUNE_COMMAND_NAME);
    const patch_command = patch_candidates.isCommand(script);
    var extra_field_name: ?[]const u8 = null;
    var extra_json: ?[]u8 = null;
    defer if (extra_json) |json| allocator.free(json);
    const snapshot = blk: {
        lockInference();
        defer unlockInference();

        if (abstraction_command) {
            ...
            const paths = global_state_paths orelse {
                sendJsonError(sock, .service_unavailable, "State shard paths unavailable");
                return;
            };
            if (reuse_command) {
                var reuse_result = abstractions.stageReuseFromCommand(allocator, paths, script) catch |err| {
                    handleAbstractionCommandError(sock, err);
                    return;
                };
                defer reuse_result.deinit();
                extra_field_name = "abstractionReuse";
                extra_json = abstractions.renderReuseJson(allocator, &reuse_result) catch {
                    sendJsonError(sock, .internal_server_error, "Failed to serialize abstraction reuse result");
                    return;
                };
            } else if (merge_command) {
                var merge_result = abstractions.stageMergeFromCommand(allocator, paths, script) catch |err| {
                    handleAbstractionCommandError(sock, err);
                    return;
                };
                defer merge_result.deinit();
                extra_field_name = "abstractionMerge";
                extra_json = abstractions.renderMergeJson(allocator, &merge_result) catch {
                    sendJsonError(sock, .internal_server_error, "Failed to serialize abstraction merge result");
                    return;
                };
            } else if (prune_command) {
                var prune_result = abstractions.stagePruneFromCommand(allocator, paths, script) catch |err| {
                    handleAbstractionCommandError(sock, err);
                    return;
                };
                defer prune_result.deinit();
                extra_field_name = "abstractionPrune";
                extra_json = abstractions.renderPruneJson(allocator, &prune_result) catch {
                    sendJsonError(sock, .internal_server_error, "Failed to serialize abstraction prune result");
                    return;
                };
            } else {
                var record = abstractions.stageFromCommand(allocator, paths, script) catch |err| {
                    handleAbstractionCommandError(sock, err);
                    return;
                };
                defer record.deinit();
                extra_field_name = "abstraction";
                extra_json = abstractions.renderJson(allocator, &record) catch {
                    sendJsonError(sock, .internal_server_error, "Failed to serialize abstraction result");
                    return;
                };
            }
            ...
            var batch = patch_candidates.stageFromCommand(allocator, paths, script) catch |err| {
                handlePatchCandidateCommandError(sock, err);
                return;
            };
            defer batch.deinit();
            extra_field_name = "patchCandidates";
            extra_json = patch_candidates.renderJson(allocator, &batch) catch {
                sendJsonError(sock, .internal_server_error, "Failed to serialize patch candidate result");
                return;
            };
        } else if (parsed_command != .none) {
            const live_scratchpad = global_scratchpad orelse {
                sendJsonError(sock, .service_unavailable, "Scratchpad unavailable");
                return;
            };
            const meaning_file = global_meaning_file orelse {
                sendJsonError(sock, .service_unavailable, "Meaning state unavailable");
                return;
            };
            const tags_file = global_tags_file orelse {
                sendJsonError(sock, .service_unavailable, "Tag state unavailable");
                return;
            };
            const meaning_words = global_meaning orelse {
                sendJsonError(sock, .service_unavailable, "Meaning words unavailable");
                return;
            };
            const tags_words = global_tags orelse {
                sendJsonError(sock, .service_unavailable, "Tag words unavailable");
                return;
            };

            _ = sigil_snapshot.executeCommand(.{
                .allocator = allocator,
                .paths = global_state_paths orelse {
                    sendJsonError(sock, .service_unavailable, "State shard paths unavailable");
                    return;
                },
                .engine = live,
        ...
        applySigilControlEffects(live, control) catch {
            sendJsonError(sock, .internal_server_error, "Failed to apply Sigil control state");
            return;
        };
        break :blk control.snapshot();
    };

    const json = buildSigilStateJson(
        allocator,
        snapshot,
        sigil_snapshot.status(global_scratchpad orelse {
            sendJsonError(sock, .service_unavailable, "Scratchpad unavailable");
            return;
        }, allocator, global_state_paths orelse {
            sendJsonError(sock, .service_unavailable, "State shard paths unavailable");
            return;
        }),
        if (abstraction_command)
            (if (reuse_command)
                ...
                abstractions.COMMIT_COMMAND_NAME)
        else if (patch_command)
            patch_candidates.COMMAND_NAME
        else if (parsed_command == .none)
            null
        else
            sigil_snapshot.commandName(parsed_command),
        extra_field_name,
        extra_json,
    ) catch {
...
fn recvExact(sock: usize, buf: []u8) bool {
    var got: usize = 0;
    while (got < buf.len) {
        const n = posix.recv(@intCast(sock), buf[got..], 0) catch return false;
        if (n == 0) return false;
        got += n;
    }
    return true;
...
fn initServerSocket(port: u16) !usize {
    const server = try posix.socket(posix.AF.INET, posix.SOCK.STREAM | posix.SOCK.CLOEXEC | posix.SOCK.NONBLOCK, posix.IPPROTO.TCP);
    errdefer posix.close(server);

    const reuse_addr: c_int = 1;
    posix.setsockopt(server, posix.SOL.SOCKET, posix.SO.REUSEADDR, std.mem.asBytes(&reuse_addr)) catch {};

    const addr = try std.net.Address.parseIp4("127.0.0.1", port);
    try posix.bind(server, &addr.any, addr.getOsSockLen());
    try posix.listen(server, 1024);
    return @intCast(server);
}

fn setSocketBlocking(sock: usize) !void {
    var flags = posix.fcntl(@intCast(sock), posix.F.GETFL, 0) catch return error.SocketConfigureFailed;
    flags &= ~SOCKET_NONBLOCK_FLAG;
    _ = posix.fcntl(@intCast(sock), posix.F.SETFL, flags) catch return error.SocketConfigureFailed;
}
...
fn serveLoop(allocator: std.mem.Allocator, server: usize) !void {
    ...
    while (!stop_flag.load(.acquire)) {
        const client_fd = posix.accept(@intCast(server), null, null, 0) catch |err| {
            if (err == error.WouldBlock) {
                sys.sleep(SHUTDOWN_POLL_MS);
                continue;
            }
            sys.print("[HTTP] Accept error: {any}\n", .{err});
            break;
        };
        const client: usize = @intCast(client_fd);
        if (stop_flag.load(.acquire)) {
        ...
        const n = posix.recv(client_fd, req_buf[0..], 0) catch {
            closeSocketNow(client);
            continue;
        };
        if (n == 0) {
