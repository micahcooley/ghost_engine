...
pub fn defaultCapabilities() [71]CapabilityEntry {
    return .{
        ...
        .{ .capability = .@"learning.review", .policy = .allowed },
        ...
        .{ .capability = .@"oracle.auto_fix", .policy = .allowed },
        .{ .capability = .@"curiosity.status", .policy = .allowed },
        .{ .capability = .@"hive.status", .policy = .allowed },
        .{ .capability = .@"recursive_boot.status", .policy = .allowed },
...
pub fn requestCapabilityName(kind: RequestKind) ?CapabilityName {
    return switch (kind) {
        ...
        .@"oracle.auto_fix" => .@"oracle.auto_fix",
        .@"curiosity.status" => .@"curiosity.status",
        .@"hive.status" => .@"hive.status",
        .@"recursive_boot.status" => .@"recursive_boot.status",
...
pub fn operationMaturityLabel(kind: RequestKind) []const u8 {
    ...
    return switch (kind) {
        ...
        .@"learning.review" => "append_only_reviewed_learning_record_no_hidden_mutation",
        .@"learning.loop.plan" => "candidate_only_project_autopsy_learning_loop_plan_no_execution_no_mutation",
        .@"oracle.auto_fix" => "explicit_temp_candidate_self_healing_oracle_no_source_mutation",
        .@"curiosity.status" => "read_only_curiosity_guard_and_speculative_candidate_status",
        .@"hive.status" => "read_only_hive_protocol_status_no_network_join",
        .@"recursive_boot.status" => "read_only_recursive_boot_measurement_no_hot_swap",
