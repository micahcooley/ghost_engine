...
pub fn main() !void {
    ...
    var intent = try task_intent.parse(allocator, text, .{ .context_target = context_target });
    defer intent.deinit();

    const rendered = try task_intent.renderJson(allocator, &intent);
...
fn printUsage() void {
    sys.print(
        "Usage: ghost_task_intent <natural-language request> [--context-target=spec]\n",
        .{},
    );
}
