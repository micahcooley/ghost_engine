...
test "validateInstalledPack happy path" {
    ...
    defer allocator.free(guidance_path);
    const guidance_data =
        \\{"schema":"ghost.autopsy_guidance.v1","packGuidance":[{"pack_id":"test_valid_pack","signals":[]}]}
    ;
    var f2 = try std.fs.createFileAbsolute(guidance_path, .{});
    defer f2.close();
    try f2.writeAll(guidance_data);

    var report = try validateInstalledPack(allocator, pack_id, pack_version);
    defer report.deinit();

    try std.testing.expect(report.ok());
}

test "validateInstalledPack missing manifest returns error report" {
    const allocator = std.testing.allocator;
...
test "validateInstalledPack missing guidance file returns structured report" {
    const allocator = std.testing.allocator;
    const pack_id = "test_missing_guidance";
    const pack_version = "v1";
