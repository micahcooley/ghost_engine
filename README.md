...
- `response_engine` exposes user-facing reasoning levels (`quick`, `balanced`, `deep`, `max`) and maps them deterministically to compute tiers (`low`, `medium`, `high`, `max`). Internal `draft_mode`, `fast_path`, `deep_path`, and `auto_path` remain policy-selected implementation details.
- Reasoning level biases mode selection, but does not force it blindly: explanation and brainstorming requests can still use draft/fast at `max`, while verify/proof/test/apply/fix-shaped requests select the proof-capable path unless the user explicitly asks for a draft.
- Draft output is always unresolved and `verificationState=unverified`; explicit verify/proof/test/correctness or patch-capable requests are forced to the proof-capable path instead of draft unless wording such as `just draft` is present.
- response debug JSON includes `requested_reasoning_level`, `effective_compute_budget_tier`, `selected_response_mode`, `mode_selection_reason`, and `user_override_detected`.
- Normal CLI usage should prefer `--reasoning=quick|balanced|deep|max`: quick is fast/minimal, balanced is default, deep is more thorough and may verify when useful, and max is most thorough without forcing deep when deep adds no value. `--compute-tier` and direct internal-mode controls are advanced/debug controls, not the primary UX.
