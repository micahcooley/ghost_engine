...
pub const VulkanEngine = struct {
    ...
    matrix_slots: u32 = 0, // NEW: Track slot capacity dynamically (Task 1)
    ...
    fn updateDescriptorSets(self: *VulkanEngine) void {
        ...
        var wds = [_]vk.VkWriteDescriptorSet{ std.mem.zeroes(vk.VkWriteDescriptorSet) } ** 5;
        for (&wds, 0..) |*w, i| {
        ...
        vk.vkUpdateDescriptorSets(self.dev, 5, &wds[0], 0, null);
    }
    ...
    pub fn init(allocator: std.mem.Allocator) !VulkanEngine {
        ...
        engine.mapped_rotors = @ptrCast(@alignCast(try engine.createBuffer(BATCH_SIZE * 8 * 2, vk.VK_BUFFER_USAGE_STORAGE_BUFFER_BIT, batch_flags, &engine.rotor_buffer, &engine.rotor_memory)));
        engine.mapped_chars = @ptrCast(@alignCast(try engine.createBuffer(BATCH_SIZE * 4, vk.VK_BUFFER_USAGE_STORAGE_BUFFER_BIT, batch_flags, &engine.char_buffer, &engine.char_memory)));
        engine.mapped_energy = @ptrCast(@alignCast(try engine.createBuffer(BATCH_SIZE * 4, vk.VK_BUFFER_USAGE_STORAGE_BUFFER_BIT, batch_flags, &engine.energy_buffer, &engine.energy_memory)));
    ...
    pub fn dispatchRecursiveLookahead(self: *VulkanEngine, num_rotors: u32, depth: u32) ![]u32 {
        var beginInfo = std.mem.zeroes(vk.VkCommandBufferBeginInfo);
        beginInfo.sType = vk.VK_STRUCTURE_TYPE_COMMAND_BUFFER_BEGIN_INFO;
        try check(vk.vkBeginCommandBuffer(self.command_buffer, &beginInfo));
        ...
        const result = try self.allocator.alloc(u32, num_rotors);
    ...
    pub fn dispatchResonance(self: *VulkanEngine, lexical_rotor: u64, semantic_rotor: u64) ![]u32 {
        ...
        const result = try self.allocator.alloc(u32, 256);
